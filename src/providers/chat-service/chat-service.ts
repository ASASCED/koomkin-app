...
export class ChatServiceProvider {
  ...
  startChatService(uuid: string) {
    this.connectClientWithUsername(uuid)
      .then(() => {
  ...
  connectClientWithUsername(username) {
    ...
    return self.fetchAccessToken(
      self.tc.username,
      self.connectMessagingClient.bind(self)
    );
  }
  ...
  connectMessagingClient(token) {
    ...
    return self.tc.messagingClient
      .initialize()
      .then(() => {
        this.chatClientStarted = true;
        self.tc.messagingClient.on("tokenExpired", () => {
          self.refreshToken();
        });
      })
      .catch(function(error) {
  ...
  mostrarAlertaEnviarSolicitudChat(
    messageTitle: string,
