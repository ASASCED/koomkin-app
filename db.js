...
db.searchDemoUsers = function(email) {
  updatequerystring =
    "select * from CATUSUARIO WHERE EMAIL = '" +
    email +
    "' AND DemoUserID IS NOT NULL;";
...
db.executeGetLeadsPagination = function(
  id,
  finicio,
  ffin,
  filtro,
...
db.executeModifyLead2 = function(
  command,
  clave,
  classification,
  canal,
    ...
    request = new Request(requestStr, function(err, rowCount) {
      if (err) {
        ...
        if (result[0] != null) {
          ...
          console.log(
            "http://localhost:5000/facebook/CampaignInsights?criteria=" +
              tipo +
              "&campaign_id=" +
              result[0].id_campaign +
              "&tipo_empresa=" +
              result[0].tipo_empresa +
              "&since=" +
              result[0].since.getFullYear() +
              "-" +
              sinceMonth +
              "-" +
              sinceDay +
              "&until=" +
              result[0].until.getFullYear() +
              "-" +
              untilMonth +
              "-" +
              untilDay
...
//getAccesoReporte
db.executeAccesoReporte = function(
  command,
  id,
  acceso,
  dispositivo,
...
//getInsertClickChat
db.executeInsertRegisterChat = function(
  command,
  usuario,
  id,
  acceso,
...
db.PostRegisterDeviceId = function(IdUsuario, IdDevice, callback) {
  ...
  connection.on("connect", function(err) {
    ...
    console.log(
      "INSERT INTO dbo.userMobileDevices (IdUsuario, IdDevice, DevicePlatform, LoginDate) VALUES ('" +
        IdUsuario +
        "','" +
        IdDevice +
        "','" +
...
db.UpdateDeviceId2 = function(IdUsuario, IdDevice) {
  updatequerystring =
    "UPDATE dbo.UserMobileDevices SET IdUsuario = " +
    IdUsuario +
    " WHERE IdDevice = '" +
    IdDevice +
    "';";
...
db.UpdateDeviceId = function(IdUsuario, IdDevice, callback) {
  ...
  connection.on("connect", function(err) {
    ...
    updatequerystring =
      ...
      IdDevice +
      "';";
    request = new Request(updatequerystring, function(err, rowCount) {
      if (err) {
...
//getCreateTicket
db.executeGenerateTicket = function(
  command,
  idUsuario,
  agente,
  fecha,
...
db.executeUpdateBriefInformation = function(
  idUsuario,
  idProducto,
  new_Producto,
  new_TipoEmpresa,
...
db.executeUpdateBriefInformation2 = function(
  idUsuario,
  idProducto,
  new_Producto,
  new_TipoEmpresa,
...
db.executeGetByClave = function(command, clave) {
  ...
  return tp.sql(requestStr).execute();
};
...
db.executeRegistrarLeadAgregado = function(
  idUsuario,
  lead_uuid,
  tipo,
  acceso,
...
//executeRegistrarLeadAgregado
db.executeRegistrarAgenda = function(
  idUsuario,
  claveLead,
  acceso,
  clasificaLead
) {
  const requestStr = `insert into ClickAgendar (IdUsuario, claveLead, FechaRegistro, Canal, ClasificaLead) values (${idUsuario}, '${claveLead}', GETDATE(), '${acceso}', '${clasificaLead}' );`;
...
db.executeGetHorarioAtencion = function(IDUSUARIO) {
  const requestStr = `SELECT * FROM HorarioAtencion WHERE idhorario IN (SELECT MAX(idhorario) FROM HorarioAtencion where IdUsuario = ${IDUSUARIO}) and IdUsuario = ${IDUSUARIO}`;
...
db.executeRegistrarClientesNuevos = function(
    idUsuario,
    ...
    const requestStr = `Update tbl_brief set ClientesNuevos = '${clientesNuevos}', tipoPublicidad = '${tipoPublicidad}', publicidadTradicional = '${publicidadTracicional}',publicidadDigital = '${publicidadDigital}' where IDUSUARIO = ${idUsuario}`
    console.log(requestStr);
...
db.updateBriefClienteParticular = function(
  clientesTargetIngresosAnuales,
  clientesTargetEdad,
  clientesTargetGenero,
  clientesTargetIntereses,
...
db.updateBriefClienteEmpresas = function(
  clientesTargetSector,
  clientesTargetCategoria,
  clientesTargetSectores,
  clientesTargetIntereses,
...
db.updateCoberturaRegion = function(idCampania, idEstado, idUsuario) {
  ...
  update TBL_BRIEF set ID_COBERTURA = 5 where IDUSUARIO = ${idUsuario}`;
...
db.executeLastUpdateMembership = function(
  RecurringPaymentID,
...
db.executeInsertComment = function(
  idUsuario,
  claveLead,
  comentario,
  clasificaLead
